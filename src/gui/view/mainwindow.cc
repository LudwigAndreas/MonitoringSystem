...
MainWindow::MainWindow(std::string &agents_folder, QWidget *parent)
  ...
  connect(ui->actionAdd_Agent, SIGNAL(triggered()), this,
          SLOT(AddAgentAction()));
...
MainWindow::~MainWindow() {
  if (log_file_ && log_file_->isOpen()) log_file_->close();
  delete log_file_;
  delete log_timer_;
  delete ui;
}
  ...
  if (item != nullptr) {
    ...
    try {
      ...
      for (auto &metric : *metrics) {
        ui->metric_list_widget->addItem(
            QString::fromStdString(metric->GetName()));
      }
      if (agent) {
...
void MainWindow::RemoveAgent(std::shared_ptr<s21::AgentBundle> &agent) {
  ...
  QListWidgetItem *agent_item =
      ui->agent_list_widget
          ->findItems(QString::fromStdString(agent->GetAgentName()),
                      Qt::MatchCaseSensitive)
          .at(0);
  delete ui->agent_list_widget->takeItem(
      ui->agent_list_widget->row(agent_item));
...
void MainWindow::OnCriticalValueReached(s21::MetricEvent event) { (void)event; }
  ...
  if (ui->agent_list_widget->currentItem() != nullptr &&
    ...
    try {
      ...
      std::chrono::duration<double> elapsedSeconds =
          currentTime - agent->GetUptimePoint();

      long hours =
          std::chrono::duration_cast<std::chrono::hours>(elapsedSeconds)
              .count();
      long minutes =
          std::chrono::duration_cast<std::chrono::minutes>(elapsedSeconds)
              .count() %
          60;
      long seconds =
...
void MainWindow::ShowAgentDetails(std::shared_ptr<s21::AgentBundle> &agent) {
  ...
  ui->agent_name_label_value->setText(
      QString::fromStdString(agent->GetAgentName()));
  ...
  ui->num_of_metrics_value_label->setText(
      QString::number(agent->GetMetrics()->size()));
  ui->uptime_value_label->setEnabled(true);
  ui->agent_type_value_line_edit->setText(
      QString::fromStdString(agent->GetAgentType()));
}

void MainWindow::ShowMetricDetails(
    std::shared_ptr<s21::ConfiguredMetric> &metric) {
  ...
  ui->metric_name_value_label->setText(
      QString::fromStdString(metric->GetName()));
  QString critical_value =
      QString::fromStdString(metric->GetCriticalValue().GetConditionOperator());
  critical_value.append(
      QString::number(metric->GetCriticalValue().GetCriticalValue()));
...
void MainWindow::SetController(
    std::shared_ptr<s21::MainController> &controller) {
  controller_ = controller;
}

void MainWindow::AddAgentAction() {
  QString agent_folder_path = QFileDialog::getExistingDirectory(
      this, tr("Select agent folder"), agents_folder_,
...
bool MainWindow::CopyDirecory(const QString &from, const QString &to) {
  ...
  QStringList fileList = sourceDir.entryList(
...
void MainWindow::on_add_agent_button_clicked() { AddAgentAction(); }
...
void MainWindow::on_pushButton_clicked() {
  if (ui->agent_list_widget->currentItem()) {
    try {
      s21::AgentBundlePtr agent =
          agents_.at(ui->agent_list_widget->currentRow());
      ...
      for (auto &metric : *agent->GetMetrics()) {
        if (ui->metric_list_widget->currentItem() &&
            metric->GetName() ==
                ui->metric_name_value_label->text().toStdString()) {
          metric->SetUpdateTime(ui->update_time_value_spin_box->value());
          metric->SetCriticalValue(s21::MetricCriticalValue(
              ui->critical_value_line_edit->text().toStdString()));
        }
      }
...
void MainWindow::on_main_tab_widget_currentChanged(int index) {
  if (index == 3) {
    ui->telegram_username_value->setText(
        QString::fromStdString(controller_->GetTelegramSender()));
    ui->email_address_value->setText(
        QString::fromStdString(controller_->GetEmailSender()));
  }
}
