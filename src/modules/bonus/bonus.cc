...
namespace s21 {
    ...
    void cpu::update_values() {
        ...
        if (name == "cpu") {
            ...
            std::transform(current_usage_.begin(), current_usage_.end(), 
                            last_usage_.begin(), cpu_usage_.begin(), std::minus<double>());
        }
        double sum = std::accumulate(cpu_usage_.begin(), cpu_usage_.end(), 0.) * .01;
        std::transform(cpu_usage_.begin(), cpu_usage_.end(), cpu_usage_.begin(), [sum](double value){return value / sum;});
        // cpu_usage() = cpu_usage_;
        // last_usage() = last_usage_;
        // current_usage() = current_usage_;
    }

    double cpu::cpu_idle_usage(){
        return cpu_usage_[cpu_type::idle];
    }         // 4

    double cpu::cpu_user_usage(){
        return cpu_usage_[cpu_type::user];    
    }         // 1
    ...
    double cpu::cpu_dpc_usage(){
        return cpu_usage_[cpu_type::nice];
    }          // ??

    double cpu::cpu_interrupt_usage(){
        return cpu_usage_[cpu_type::irq];    
    }    // 6

    
    std::map<std::string, std::string> memory::meminfo = std::map<std::string, std::string>(); 
    ...
    double memory::free_ram() {
        return get_value("MemAvailable");
    }

    double memory::total_swap() {
        return get_value("SwapTotal");
    }

    double memory::free_swap() {
...
std::string proc_queue_length() {
    ...
    command >> answer;
    ss << std::fixed << answer - 1; // Subtracting 1 because ps call also counts as runnable
    return ss.str(); 
}
...
// that weird regex is parsing date in format "mmm dd"
std::string system_errors() { 
    int             error_count;
    std::stringstream ss;
    redi::ipstream  command("journalctl -p 3 -b -q | grep -E '[A-Z][a-z]{2} [0-9]{2}' | wc -l");
