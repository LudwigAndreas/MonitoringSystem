...
EmailAddress::operator const char *() const { return email_.c_str(); }

std::ostream &operator<<(std::ostream &out, const EmailAddress &email) {
  if (!email.name_.empty()) out << email.name_ << ' ';
...
std::ostream &operator<<(std::ostream &out,
  ...
  if (!emailAddresses.empty()) {
    ...
    for (auto it = std::next(emailAddresses.cbegin());
         it != emailAddresses.end(); it = std::next(it)) {
      out << ", " << *it;
    }
  }
  return out;
