...
AgentExecutor::AgentExecutor(MetricAnalyzerPtr &logger) { logger_ = logger; }

void AgentExecutor::OnAgentAdded(std::shared_ptr<AgentBundle> &agent) {
  agent_schedulers_.insert(
      {agent, std::make_shared<AgentScheduler>(agent, logger_)});
}
...
AgentExecutor::GetAgentSchedulers() {
