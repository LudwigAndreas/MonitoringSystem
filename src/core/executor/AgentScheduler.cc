...
AgentScheduler::AgentScheduler(AgentBundlePtr agent, MetricAnalyzerPtr analyzer) : running_(false) , analyzer_(analyzer) {
  for (const auto& metric : *agent->GetMetrics()) {
    metrics_.push_back({metric});
  }
  if (metrics_.empty())
    LOG_WARN(diagnostic::Logger::getRootLogger(), "AgentScheduler: No metrics found for agent: " << agent->GetAgentName())
  ...
  while (running_) {
      for (auto& metric_data : metrics_) {
        ...
        if (std::chrono::system_clock::now() >= metric_data.scheduled_time) {
          ...
          metric_data.scheduled_time = std::chrono::system_clock::now() + std::chrono::seconds(interval);
        }
      }
