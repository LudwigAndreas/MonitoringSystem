...
AgentBundle::AgentBundle(const AgentPtr &agent, const PropertiesPtr &properties,
  ...
  for (const Metric &metric : agent->GetMetrics()) {
    configured_metrics_->emplace_back(
        std::make_shared<ConfiguredMetric>(metric));
  }
  Configure(properties);
}

AgentBundle::~AgentBundle() { delete configured_metrics_; }
...
std::string AgentBundle::GetAgentName() const { return name_; }

std::string AgentBundle::GetAgentType() const { return type_; }

const std::string &AgentBundle::GetName() const { return name_; }
void AgentBundle::SetName(const std::string &name) { name_ = name; }
const std::string &AgentBundle::GetType() const { return type_; }
void AgentBundle::SetType(const std::string &type) { type_ = type; }
...
void AgentBundle::ConfigureMetric(const PropertiesPtr &properties,
  ...
  std::string critical_value =
      properties->GetProperty("metric." + metric_name + ".critical_value", "");

  std::string update_interval =
      properties->GetProperty("metric." + metric_name + ".update_time", "");

  std::string metric_args =
      properties->GetProperty("metric." + metric_name + ".args", "");
  ...
  if (!critical_value.empty()) {
    ...
    LOG_WARN(diagnostic::Logger::getRootLogger(),
             "Critical value for metric " + metric_name + " is not set");
  }
  ...
  if (!update_interval.empty()) {
    ...
    LOG_WARN(diagnostic::Logger::getRootLogger(),
             "Update interval for metric " + metric_name + " is not set");
  }
...
void AgentBundle::SetUptimePoint(
    std::chrono::system_clock::time_point uptime_point) {
  AgentBundle::uptime_point_ = uptime_point;
}

bool AgentBundle::IsEnabled() const { return is_enabled_; }

void AgentBundle::SetIsEnabled(bool is_enabled) {
  AgentBundle::is_enabled_ = is_enabled;
}

time_t AgentBundle::GetLastModified() const { return last_modified_; }
...
const std::string &AgentBundle::GetAgentPath() const { return agent_path_; }
