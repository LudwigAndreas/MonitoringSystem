...
class EmailAddressesTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    // You can add setup code here if needed
    // For example, populate the EmailAddresses vector with sample EmailAddress
    // objects
    emailAddresses.push_back(
        s21::EmailAddress("user1@example.com", "User One"));
    emailAddresses.push_back(
        s21::EmailAddress("user2@example.com", "User Two"));
  }
};
