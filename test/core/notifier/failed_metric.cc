...
#include "../../../src/core/notifier/FailedMetric.h"  // include the header file for your s21::failed_metric class
...
class FailedMetricTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    failedMetric1 =
        s21::FailedMetric("Metric1", "Critical1", "Value1", "Date1");
    failedMetric2 =
        s21::FailedMetric("Metric2", "Critical2", "Value2", "Date2");
  }
};
...
TEST_F(FailedMetricTest, ConstructorWithMetricEvent) {
  ...
  std::string str = "null";
  auto tim = time(NULL);
  s21::MetricEvent metricEvent(ptr, str, tim);
