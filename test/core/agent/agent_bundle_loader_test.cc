 ...
 protected:
  void SetUp() override {
    ...
    try {
      ...
      agent_path = std::filesystem::absolute(parent_path).string() + "/tmp-" +
                   path_to_agent.filename().string();
      std::filesystem::create_directory(agent_path);
      std::filesystem::copy(test_module_path, agent_path,
  ...
  void TearDown() override { std::filesystem::remove_all(agent_path); }
...
TEST_F(AgentBundleLoaderTest, LoadAgent) {
  std::shared_ptr<s21::AgentBundle> agent_bundle =
      s21::AgentBundleLoader::LoadAgentBundle(agent_path);
...
TEST_F(AgentBundleLoaderTest, LoadBrokenAgent) {
  ...
  std::shared_ptr<s21::AgentBundle> empty_agent =
      s21::AgentBundleLoader::LoadAgentBundle(agent_path);
...
TEST_F(AgentBundleLoaderTest, UpdateAgent) {
  ...
  std::shared_ptr<s21::AgentBundle> updated_agent_bundle =
      s21::AgentBundleLoader::UpdateAgentBundle(agent_path, agent_bundle);
  ...
  EXPECT_LE(
      updated_agent_bundle->GetLastModified(),
      std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
