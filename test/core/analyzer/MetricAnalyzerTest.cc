...
TEST_F(MetricAnalyzerTest, NewFileOnNewDay) {
  ...
  expected_filename << log_dir << "/" << std::put_time(std::localtime(&timestamp), "%m_%d_%Y")
           << ".log";
  std::ostringstream line;
  line << "[" << std::put_time(std::localtime(&timestamp), "%H:%M:%S") << "] | "
  << metric->GetName() << "<" << metric->GetArgs() << ">"
  << " : " << value << " | ";
  EXPECT_CALL(*mock_subscriber_, OnNewFileOpened(expected_filename.str())).Times(1);
...
TEST_F(MetricAnalyzerTest, Unsubscribe) {
  ...
  line << "[" << std::put_time(std::localtime(&timestamp), "%H:%M:%S") << "] | "
       ...
       << " : " << value << " | ";
  metric_analyzer_->Unsubscribe(mock_subscriber_.get());
  EXPECT_CALL(*mock_subscriber_, OnNewFileOpened(expected_filename.str())).Times(0);
